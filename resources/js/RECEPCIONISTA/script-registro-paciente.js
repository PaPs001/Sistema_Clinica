...
function handlePatientRegistration(e) {
    ...
    const patientData = {
        ...
        id_number: formData.get('patient-id'), // Changed key to match controller
        ...
        blood_type: formData.get('patient-blood-type'), // Changed key to match controller
        ...
        .then(data => {
            if (data.success) {
                ...
                if (data.errors) {
                    ...
                    // Aquí podrías mostrar los errores específicos en el formulario si lo deseas
                }
            }
        })
        .catch(error => {
